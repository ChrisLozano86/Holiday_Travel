...
class DestinosUno {
    ...
    const TABLA = 'categoria_uno_circuitos';
    ...
    public static function recuperarPublicados() {
        $conexion = new Conexion();
        $consulta = $conexion->prepare('SELECT * FROM categoria_uno_circuitos WHERE visible = 1 ORDER BY idSlider DESC');
        $consulta->execute();
        $registros = $consulta->fetchAll();
    ...
    public static function busqueda($search) {
        ...
        $consulta = $conexion->prepare("SELECT * from categoria_uno_circuitos WHERE (keywords LIKE '%$search%') AND visible = 1");
        $consulta->execute();
        $registros = $consulta->fetchAll();
