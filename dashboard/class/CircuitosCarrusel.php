...
class CircuitosCarrusel {
    ...
    const TABLA = 'circuitos_carrusel';
    ...
    public static function recuperarPromocionesPublicadas() {
        $conexion = new Conexion();
        $consulta = $conexion->prepare('SELECT * FROM circuitos_carrusel WHERE visible = 1 ORDER BY idPromocion DESC');
        $consulta->execute();
        $registros = $consulta->fetchAll();
